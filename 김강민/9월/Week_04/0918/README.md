...
겉으로 보기엔 숫자를 이어붙이는것 같지만, int를 str로 변환한 후, 다시 int로 변환해서 더하는 문제이다.

1. num_list에 담긴 숫자들을 짝수와 홀수로 분류하여 even_num과 odd_num으로 분류한다.
2. 이때 even_num과 odd_num은 문자열로 초기화 한다.
3. 입력받은 문자들을 다시 int로 바꾸어 짝수를 이어붙인 수와 홀수를 이어붙인 수를 더한후 return 한다.
